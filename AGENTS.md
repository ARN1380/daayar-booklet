...
### 2026-09-18 — ahoorazarivan booklet completed + ارتباطات games shortened (DONE)

1. **Asked for:** fill every missing piece of `booklets/ahoorazarivan.json` so it
   is as complete as `arman-daliri` (it had empty skill bullets, an empty 4th
   game, a missing game emoji, and text still naming the other child), and make
   the **ارتباطات** games' step descriptions a bit shorter.

2. **Plan / approach:** the file was structurally valid (the builder only checks
   structure), so nothing failed loudly — these were content gaps. Filled them
   from each domain's own `intro` (the intro says what the child is doing, the
   bullets list it): 4–5 bullets per domain, in arman's short-bullet style. The
   empty 4th حل مسئله game got a real game («تکه‌ها رو کنار هم بذار») and the
   emoji-less «کجا بذارمش؟» got 📦, because `bookPages.tsx` splits that domain
   2+2 over pages 13/14 and an empty card is visibly broken. Name/digit cleanups:
   `آرمان`→`اهورا`, the `اهور`/`راوکامل` typos, Persian digits for the dates +
   `۳۰ ماهگی` (arman uses ۱۲ ماهگی), `شخصی - اجتماعی`→`شخصی ـ اجتماعی`, and the
   missing `🎯` prefix on one goal line. Then the three ارتباطات game steps were
   trimmed ~35% — kept the instruction and the «فکر می‌کنی بعدش چی میشه؟»-style
   prompts, dropped the hedging clauses. That trim is also what made the page fit
   (see Verified).

3. **Done:**
   - `booklets/ahoorazarivan.json` — bullets filled in all 5 `domainSkills`;
     `domainGames[3]` completed (game 3 emoji 📦, new game 4 with
     emoji/title/steps); `gamesIntro`, `reminder.lines` and `nextStep` no longer
     mention آرمان; dates/age in Persian digits; the 3 ارتباطات game steps
     shortened.
   - `src/data/content-map.ts` — regenerated by `npm run content`.

4. **Verified:** `npm run content` ✓ (3 booklets), `npx tsc --noEmit` ✓,
   `npm run lint` ✓, `npm run build` ✓ (`/ahoorazarivan` SSG, routes unchanged).
   CDP probe over all 20 flip-book pages at 1920×1080 (`next start` on a fresh
   port, tour flag set first): **0 pages overflow their page box**, and DOM
   assertions confirmed the new bullet, the new 4th game, the shortened wording,
   and **no remaining «آرمان»/«اهور » anywhere in the booklet**. The first draft of
   the shortened ارتباطات page measured `bottom+4px` while arman's same page
   measures `bottom+0` — hence the extra trim; a content-only edit still has to
   be probed. Probe file and both test servers removed afterwards.

5. **Left to do:** nothing for ahoorazarivan. Note `booklets/qwer.json` still
   exists as a near-empty test booklet and is reachable at `/qwer` — delete it if
   it was only ever a scratch draft.

6. **Traps:**
   - A content edit can silently overflow a page even though `npm run build`
     passes: the builder validates counts/structure only, never fit. Re-run the
     §8 CDP probe for any page whose text got longer.
   - `document.body.innerText` only covers the pages StPageFlip is currently
     showing (the rest are `display: none`); use `textContent` when asserting a
     string exists somewhere in the booklet.
   - The 4 games of the 4th domain are split 2+2 by `bookPages.tsx`; an empty
     game there renders as a blank card with just «−», not an error.

### 2026-09-18 — Diagnosed why saving fails on the live Vercel site (DONE — user action required)

1. **Asked for:** "we cant create new booklets on production on vercel website.
   i tried to connect it to github but i think i wasnt successful." English-only
   follow-up.

2. **Plan / approach:** no code changes — the GitHub-commit save (`735f713`) was
   already deployed. Instead, probe the **live** site to separate "stale build"
   from "env missing". Evidence chain:
   - repo `origin` = `ARN1380/daayar-booklet`, head `e0e2025` pushed 2026-09-14,
     so `main` contains the GitHub-save route (idempotent from GitHub API).
   - `GET /admin/api/save` → 405 (route present), `GET /` → 200,
     `GET /ahoorazarivan` → 200, proving the deployment is >= `e0e2025`
     (ahoora only exists there) — i.e. NOT a stale/pre-GitHub-save build.
   - POST a full valid booklet (manifest of `arman-daliri.json`) under
     `slug:"zz-diagnose"` to `https://daayar-booklet.vercel.app/admin/api/save`
     → **HTTP 500, «ذخیره در پوشهی booklets/ ممکن نشد (دیسک فقطخواندنی
     است؟)…»**, the local-`fs` fallback branch of `route.ts`. That branch runs
     **only when `process.env.GITHUB_TOKEN` is empty**, so the conclusion is
     exact: the current running deployment has NO `GITHUB_TOKEN`.
   - The 500 means nothing was written (no junk file left in the repo).

3. **Done:** nothing in the codebase (working tree still clean). Diagnosis above.

4. **Verified:** the three live probes above (405/200/200) + the 500 with the
   exact fs-fallback Persian message.

5. **Left to do (user-side, in the Vercel dashboard):** the env var is not
   reaching the production runtime. Check, in order: (a) `GITHUB_TOKEN` exists
   and is selected for the **Production** environment (Settings → Environment
   Variables — Production/Preview/Development checkboxes; it's easy to add it
   for Development/Preview only), (b) the token is a real PAT (fine-grained with
   **Contents: Read and write** on `ARN1380/daayar-booklet`, or classic with
   `repo` scope), (c) a **redeploy** happens after adding/editing the env var —
   Vercel env changes do NOT affect the currently running deployment until the
   project is redeployed (push any commit or click Redeploy). Then re-test the
   same POST (a success returns `mode:"github"` and creates
   `booklets/zz-diagnose.json` in the repo — delete that test file afterwards,
   or better, just click «ذخیره در پروژه» on a real booklet and expect the
   «به GitHub ارسال شد و سایت بهزودی بهصورت خودکار بهروز میشود» message).

6. **Traps:** don't re-test with a valid slug until the env var fix is deployed,
   or a successful POST will commit a junk `booklets/zz-diagnose.json` and
   trigger a Vercel redeploy. The stored default `GITHUB_REPO` is
   `ARN1380/daayar-booklet` which matches the actual repo — no override needed.
