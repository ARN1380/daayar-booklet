...
|---|---|
| `npm run dev` | Development server with hot reload. Regenerates the booklet content from `booklet.txt` first. |
| `npm run build` | Production build. **Run this before committing** — it is the type check. |
| `npm run content` | `booklet.txt` → `src/data/content.ts`. Runs automatically before `dev` and `build`. |
...
│   └── content.ts        AUTOGENERATED from booklet.txt — do not hand-edit
├── lib/fa.ts             toFaDigits() — Western → Persian numerals (۱۲۳)
booklet.txt               THE source of all Persian text (Notepad-editable)
scripts/
├── build-booklet.mjs     booklet.txt → src/data/content.ts (the real parser)
└── export-booklet.mjs    one-time seeding tool (not part of the normal build)
```
...
All Persian wording lives in **`booklet.txt`** at the repo root — a plain,
Notepad-editable file. The structure is documented at the top of that file (Persian
`;;` comments). A build script (`npm run content`) compiles it into
`src/data/content.ts` automatically whenever you run `npm run dev` or
`npm run build`.

### Quick start for editing

1. Open `booklet.txt` in Notepad (or any editor).
2. Find the field you want to change (child info, statuses, results, skills,
   games, reminder, next step) — each is under a labeled `####` heading.
3. Change the text. Keep every `##` / `###` / `####` heading exactly as-is.
4. Save, then run `npm run dev` (or `npm run build` to deploy).

The script validates the text against the fixed page layout (5 domains, game
counts 3/3/3/4/3, ≤6 skills, ≤5 steps per game) and tells you if anything
breaks, in Persian with line numbers.
...
3. Each list item under `#### عنوان` or `#### متن` is a single paragraph.
   If you wrap a long line in Notepad, keep it as one logical line (Notepad's
   word wrap does not insert newlines; save as-is).
4. Do not change the `##` / `###` / `####` heading lines themselves — they are
   the structure the parser depends on.
5. If you change text that is *long*, re-check the page in the browser — pages
...
6. Changing the child's name in `booklet.txt` automatically updates the cover,
   info pages, closing page, guided tour welcome and browser tab title — no need
   to hunt down every occurrence.

### Adding/removing a page

1. Add the new text to `booklet.txt` (follow the existing section format).
